#!/bin/bash
#
# Ralph Loop for Claude Code via Ollama
#
# Based on Geoffrey Huntley's Ralph Wiggum methodology.
# Combined with SpecKit-style specifications.
#
# Usage:
#   ./scripts/ralph-loop-ollama.sh              # Build mode (unlimited)
#   ./scripts/ralph-loop-ollama.sh 20           # Build mode (max 20 iterations)
#   ./scripts/ralph-loop-ollama.sh plan        # Planning mode (optional)
#
...
# Spec queue helper functions (inlined)
count_root_specs() {
    local spec_dir="$1"
    [ -d "$spec_dir" ] && find "$spec_dir" -maxdepth 1 -name "*.md" -type f | wc -l || echo 0
}

count_incomplete_root_specs() {
    local spec_dir="$1"
    if [ -d "$spec_dir" ]; then
        find "$spec_dir" -maxdepth 1 -name "*.md" -type f -exec grep -L "Status:.*COMPLETE" {} \; | wc -l
    else
        echo 0
    fi
}
...
get_first_incomplete_root_spec() {
    local spec_dir="$1"
    if [ -d "$spec_dir" ]; then
        find "$spec_dir" -maxdepth 1 -name "*.md" -type f -exec grep -L "Status:.*COMPLETE" {} \; | head -1 | xargs basename 2>/dev/null || echo ""
    else
        echo ""
    fi
}
...
Usage:
  ./scripts/ralph-loop-ollama.sh              # Build mode, unlimited iterations
  ./scripts/ralph-loop-ollama.sh 20           # Build mode, max 20 iterations
  ./scripts/ralph-loop-ollama.sh plan         # Planning mode (optional)
...
Prerequisites:
  ...
  - Model pulled (e.g., ollama pull minimax-m2.5:cloud)
  - Set OLLAMA_MODEL env var to use a different model
...
SESSION_LOG="$LOG_DIR/ralph_ollama_${MODE}_session_$(date '+%Y%m%d_%H%M%S').log"
...
# Configure Claude to use Ollama as the API provider
export ANTHROPIC_AUTH_TOKEN=ollama
export ANTHROPIC_API_KEY=""
export ANTHROPIC_BASE_URL="${ANTHROPIC_BASE_URL:-http://localhost:11434}"

echo -e "${BLUE}Using Ollama model:${NC} $OLLAMA_MODEL"
echo -e "${BLUE}Ollama endpoint:${NC} $ANTHROPIC_BASE_URL"
...
echo ""
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}        RALPH LOOP (Ollama/Claude) STARTING                 ${NC}"
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
echo -e "${BLUE}CLI:${NC}      $CLAUDE_CMD --model $OLLAMA_MODEL"
...
while true; do
    ...
    LOG_FILE="$LOG_DIR/ralph_ollama_${MODE}_iter_${ITERATION}_$(date '+%Y%m%d_%H%M%S').log"
    : > "$LOG_FILE"
    WATCH_PID=""
    ...
    # Run Claude via Ollama with prompt via stdin, capture output
    CLAUDE_OUTPUT=""
    if CLAUDE_OUTPUT=$(cat "$PROMPT_FILE" | "$CLAUDE_CMD" --model "$OLLAMA_MODEL" $CLAUDE_FLAGS 2>&1 | tee "$LOG_FILE"); then
        if [ -n "$WATCH_PID" ]; then
        ...
        if echo "$CLAUDE_OUTPUT" | grep -qE "<promise>(ALL_)?DONE</promise>"; then
            ...
            if [ "$MODE" = "plan" ]; then
                ...
                echo -e "${CYAN}Run './scripts/ralph-loop-ollama.sh' to start building.${NC}"
        ...
        echo -e "${YELLOW}Check log: $LOG_DIR${NC}"
        CONSECUTIVE_FAILURES=$((CONSECUTIVE_FAILURES + 1))
        print_latest_output "$LOG_FILE" "Claude"
    fi
...
echo ""
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}       RALPH LOOP (Ollama) FINISHED ($ITERATION iterations)  ${NC}"
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
